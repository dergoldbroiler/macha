...
function query_posts_and_pages($arg){
	$args = array(
		'post_type' => array('post', 'page', 'faq-element','manual-element'),
		'showposts' => '100',
		's' => $arg,
		'order' => 'DESC',
		'orderby' => 'date',
...
function add_manual_api(){
	register_rest_route('wp/v2', '/manual', array(
		'methods' => 'GET',
		'callback' => 'query_manuals_with_params'
	) );
}
function query_manuals_with_params($request) {
	$arg = $request->get_param( 's' );
	return query_manuals($arg);
}
function query_manuals($arg){
	$args = array(
